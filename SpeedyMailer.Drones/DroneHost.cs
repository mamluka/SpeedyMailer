        ...
        public void Initialize(int redirectedToListeningPort = 8080, bool noTasks = false)
        {
            if (!noTasks)
            {
                var tasks = new List<ScheduledTask>
				{
					...
					new SendDroneStateSnapshotTask(),
					new ResumePausedGroupsTask(),
                    new ResumeSendingForIndividualDomains(),
					new AddIndexesToMongoTask()
				};

                _framework.StartTasks(tasks);
            }

            var nancyUrl = Regex.Match(_droneSettings.BaseUrl, ":\\d{1}").Success ? _droneSettings.BaseUrl : string.Format("{0}:{1}", _droneSettings.BaseUrl, redirectedToListeningPort);

            _nancy = new NancyHost(new Uri(nancyUrl), _nancyBootstrapper);
        }
