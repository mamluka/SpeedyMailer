...
namespace SpeedyMailer.Tests.Core.Integration.Base
{
	public class DroneActions : ActionsBase
	{
		...
		public void WaitForChangeOnStoredObject<T>(Func<T, bool> condition, Func<T, bool> func, int waitFor = 30) where T : class
		{
			...
			while (!func(manager.AsQueryable.Where(condition).SingleOrDefault()) && st.ElapsedMilliseconds < waitFor * 1000)
			{
				Thread.Sleep(500);
			}

			func(manager.AsQueryable.Where(condition).SingleOrDefault()).Should().BeTrue();
		...
		public T Find<T>(Func<IQueryable<T>, T> func, string collectionName = null) where T : class
		{
			var manager = new GenericRecordManager<T>(Kernel.Get<DroneSettings>().StoreHostname, collectionName);

			return func(manager.AsQueryable);
		}

		public T LoadById<T>(string id, string collectionName = null) where T : class
		{
			var manager = new GenericRecordManager<T>(Kernel.Get<DroneSettings>().StoreHostname, collectionName);
		...
		public void WaitForIndexesToExist()
		{
			var manager = new GenericRecordManager<CreativePackage>(Kernel.Get<DroneSettings>().StoreHostname);

			var indexes = manager.Collection.GetIndexes();

			var st = new Stopwatch();
			st.Start();

			while (st.ElapsedMilliseconds < 30 * 1000 && !indexes.Any())
			{
				Thread.Sleep(500);
				indexes = manager.Collection.GetIndexes();
			}

			st.Stop();
			indexes.Should().NotBeEmpty("No indexes found");
		}
