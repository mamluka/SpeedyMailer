...
namespace SpeedyMailer.Master.Service.Storage.Indexes
{
	public class Creative_DeferredEmails : AbstractIndexCreationTask<DroneStateSnapshoot, Creative_DeferredEmails.ReduceResult>
	{
		...
		{
			Map = snapshots => snapshots.SelectMany(x => x.MailDeferred.Select(m => new { m.CreativeId, m.DomainGroup, m.Recipient, m.Time, m.Message }), (snapshot, p) => new
				{
			...
			Reduce = result => result
								   ...
								   .Select(
									   x => new
										   {
											   CreativeId = x.Key,
											   Deferred = x.SelectMany(m => m.Deferred).Select(m => new { m.DomainGroup, m.Recipient, m.Time, m.CreativeId, m.Message }),
										   });
		}
	}
}
